...
public class MsgParser {
    ...
    private static LinkedList<Coord> convertSnakeOffsetsCoords(List<Coord> offsets, int width, int height) {
        ...
        if (!offsets.isEmpty()) {
            ...
            while (!offsets.isEmpty()) {
                ...
                while (mod > 0) {
                    ...
                    temp.setX((prevCoord.getX() + offsetCoord.getX()));
                    temp.setY((prevCoord.getY() + offsetCoord.getY()));

                    if (temp.getX() < 0) {
                        temp.setX(temp.getX() + width);
                    } else if (temp.getX() > width - 1) {
                        temp.setX(temp.getX() - width);
                    }

                    if (temp.getY() < 0) {
                        temp.setY(temp.getY() + height);
                    } else if (temp.getY() > height - 1) {
                        temp.setY(temp.getY() - height);
                    }
